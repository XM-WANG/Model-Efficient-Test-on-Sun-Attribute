...
class Evaluator(object):
        ...
        for i, (imgs, img_names, targets) in enumerate(data_loader):
            ...
            if (i + 1) % print_freq == 0:
                print('Extract Features: [{}/{}]\t'
                      'Time {:.3f} ({:.3f})\t'
                      'Data {:.3f} ({:.3f})\t'.format(
                          i + 1, len(data_loader), batch_time.val,
    ...
    def evaluate(self, data_loader):
        ...
        label_mat = torch.cat(
            [labels[key].unsqueeze(0) for key in labels.keys()], 0)
        result_mat = torch.cat(
            [results[key].unsqueeze(0) for key in labels.keys()], 0)
        ...
        print('Top 5 Prec {:.2%} \t'
              'Top 5 Recall {:.2%} \t'.format(top5_prec, top5_recall))
    ...
    def test(self, data_loader, topk=(1, 3, 5)):
        ...
        print('  Prec: \t Top_1: {:.2%} \t Top_3: {:.2%} \t Top_5: {:.2%}\n'
              ...
              format(prec[0], prec[1], prec[2], recall[0], recall[1],
                     recall[2]))
